...
# c.execute("SELECT MIN(sent_id),MAX(sent_id) FROM sent_summary")
...
phen_last_sent = str(identity)
phen_source = str(identity)
phen_target = str(identity)
fillColor = str(random.choice(fill))
for sent_order in sentList:
    ...
    for row in c.fetchall():
        ...
        # For the first Phen in the sentence, store the sentence starter for the next sentence
        if id_flag == False:
            link_obj = '{source: \"' + str(phen_last_sent) + '\", target: \"' + phen_target + '\", fill: \"' + fillColor + '\"}'
            phen_last_sent = str(phen_target)
            phen_source = str(phen_target)
            #output += str(link_obj) + ",\n"
...
#output += '{source: \"' + str(phen_last_sent) + '\", target: \"' + identity + '\", fill: \"' + fillColor + '\"}];'
